...
- 再審査コースの進捗:
  ...
  - ✅ **マルチ自治体化・東京7区567丁目に拡大**（目黒88・品川137・台東108・江東58・文京33・中央56・大田87）。区セレクタ＋区ごとの分別区分。すべて東京都オープンデータ（CC BY）由来。2026-07-12に本番反映済み（PR #1 マージ）。
...
- **東京23区の開放データ現状（2026-07-12調査）**: 収集**日**の機械可読データがあるのは上記7区のみ。他の多くの区（中野など）は集積所位置・分別品目しか公開しておらず**収集日データが無い**＝open dataでの全23区対応は不可。隔週収集の区は基準週データが無く算出不能。
