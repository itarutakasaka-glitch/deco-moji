...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const ogImage = `/trash-fortune/og/${slug}`;
  return {
    title,
    description,
...
export default async function TrashFortuneResultPage({ params }: Props) {
  const { slug } = await params;
  const p = decodeSlug(slug);
  if (!p) redirect("/trash-fortune");
  const f = buildFortune(p);
